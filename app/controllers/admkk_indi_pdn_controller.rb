class AdmkkIndiPdnController < ApplicationController
	...
	def index
		#cari semua data bidang pemerintahan dalam negeri
		@indikators = Indikator.where(" bidang_id = 4 ")
	...
	def grid_data
		...
		#semua data
		total = VNilaiIndikator.where("bidang_id = 4 and provinsi_id = ? and kabkot_id = ?", current_user.provinsi_id, current_user.kabkot_id).size

		@indis = VNilaiIndikator.where("bidang_id = 4 and provinsi_id = ? and kabkot_id = ?", current_user.provinsi_id, current_user.kabkot_id).order(ordexx).limit(rows).offset(startx)
	...
	def grid_child
		...
		while i < 2016
			jum = NiPdn.where("provinsi_id = ? and kabkot_id = ? and tahun = ?", current_user.provinsi_id, current_user.kabkot_id, i).count

			if(jum < 1)
				query = "insert into ni_pdns (provinsi_id,kabkot_id,tahun) values ('"+current_user.provinsi_id.to_s+"','"+current_user.kabkot_id.to_s+"','"+i.to_s+"')"
				ActiveRecord::Base.connection.execute(query)
			end
	...
	def child_data
		...
		#semua data
		total = NiPdn.where("provinsi_id = ? and kabkot_id = ?", current_user.provinsi_id, current_user.kabkot_id).size

		@indis = NiPdn.where("provinsi_id = ? and kabkot_id = ?", current_user.provinsi_id, current_user.kabkot_id).order(ordexx).limit(rows).offset(startx)
	...
	def judul_field(kolom)
		qb = "SELECT a.COLUMN_NAME, a.COLUMN_COMMENT FROM  information_schema.COLUMNS a WHERE a.TABLE_NAME = 'ni_pdns' and a.COLUMN_NAME = '"+kolom+"'"
